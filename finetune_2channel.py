...
class TwoChannelImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        img_3channel = cv2.imread(self.image_paths[idx], cv2.IMREAD_COLOR)
        img_2channel = img_3channel[:, :, :2]
        
        img_0 = Image.fromarray(img_2channel[:, :, 0]).convert('L')
        img_1 = Image.fromarray(img_2channel[:, :, 1]).convert('L')
        
        if self.transform:
            img_0 = self.transform(img_0)
            img_1 = self.transform(img_1)
        
        img_2channel_tensor = torch.cat([img_0, img_1], dim=0)
        img_3channel_tensor = torch.cat([img_2channel_tensor, torch.zeros_like(img_0)], dim=0)
        
        label = self.labels[idx]
        return img_3channel_tensor, label
...
def load_data(data_dir):
    ...
    for json_file in sorted(data_dir.glob("*.json")):
        ...
        img_file = data_dir / f"{folder_name}.png"
...
def finetune_2channel(data_dir="processed/train", epochs=1, batch_size=32, lr=0.001, arch_name='mobilenet_v3_small'):
    ...
    with open('train_val_split_2channel.json', 'w') as f:
    ...
    train_dataset = TwoChannelImageDataset(train_paths, train_labels, transform=transform_train)
    val_dataset = TwoChannelImageDataset(val_paths, val_labels, transform=transform_val)
    ...
    for epoch in range(epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save({
                ...
                'arch_name': arch_name
            }, f'{arch_name}_2channel_finetuned.pth')
    ...
    with open(f'benchmark_results_{arch_name}_2channel.json', 'w') as f:
    ...
    print(f"\nResults saved to: benchmark_results_{arch_name}_2channel.json")
...
if __name__ == "__main__":
    ...
    finetune_2channel(arch_name=arch_name)
