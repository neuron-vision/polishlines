...
class AngleDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Images are saved as 3-channel (Grey, Highpass, Mask)
...
def load_all_samples():
    samples = []
    # Search for all data.json files in cnn_preprocessed
...
class HRNetClassifier(nn.Module):
    def __init__(self, model_name=meta_params.get('hrnet_model', 'hrnet_w48'), dropout=0.4):
        super().__init__()
        # Load HRNet backbone
...
def eval_sample_level(model, samples, transform, device, agg='max'):
    model.eval()
    correct, total = 0, 0
    tp, fp, tn, fn = 0, 0, 0, 0
    with torch.no_grad():
...
if __name__ == "__main__":
    ...
    if not all_samples:
        print("No samples found. Run preprocess_all_scan_lines.sh first.")
        exit(1)
    ...
    model = HRNetClassifier(dropout=meta_params['cnn_dropout']).to(device)
    ...
    scheduler = optim.lr_scheduler.OneCycleLR(optimizer, max_lr=meta_params['cnn_lr']*5, 
                                              epochs=meta_params['cnn_epochs'], steps_per_epoch=len(train_loader))
    ...
    for epoch in range(meta_params['cnn_epochs']):
        ...
        vm = eval_sample_level(model, val_samples, val_transform, device)
        
        print(f"E{epoch+1:03d} TL:{tl:.4f} TA:{ta:.3f} VL:{vl:.4f} VA:{va:.3f} S-Acc:{vm['acc']:.3f} F1:{vm['f1']:.3f}")
        
        if vm['f1'] > best_f1:
            best_f1 = vm['f1']
            torch.save(model.state_dict(), "best_hrnet.pt")
            print(f"  New best F1: {best_f1:.3f}")
            patience = 0
        else:
            patience += 1
            if patience >= meta_params['cnn_patience']:
